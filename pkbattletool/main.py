...
from mylib import OcrRunner, CameraCapture, OcrControl
...
def gui_main():
    ...
    # フレームウェジットの作成
    # ポケモン情報簡易表示
    frame_pkinfo = PkInfo_OCR(root, ocr_control, bd=1, relief=tk.SOLID)
    ...
    # キャプチャコントロール
    frame_capturecontrol = CaptureControl(root, camera_capture, ocr_control, bd=1, relief=tk.SOLID)
    frame_capturecontrol.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W+tk.E+tk.N+tk.S)
    
    # 相手の手持ち一覧
    frame_canvaspkbox = CanvasPkBox(root, camera_capture, ocr_control, bd=2, relief=tk.SOLID)
    ...
    def click_close():
        ...
        frame_pkinfo.ocr_control.stop_ocr_thread()
        frame_canvaspkbox.ocr_control.stop_ocr_thread()
